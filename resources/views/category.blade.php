...
@section('content')
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-2">
                <form action="{{ route('product.category', $name) }}" method="GET">
                    <!--foreach subcategories-->
                    @foreach ($subcategories as $subcategory)
                        <p><input type="checkbox" name="subcategory[]" value="{{ $subcategory->id }}"
                                @if (isset($filterSubCategories)) {{ in_array($subcategory->id, $filterSubCategories) ? 'checked ="checked" ' : '' }} @endif>{{ $subcategory->name }}
                        </p>
                        <!--end foreach-->
                    @endforeach
                ...
                <hr>
                <a href="{{ route('product.category', $name) }}">Refresh</a>
